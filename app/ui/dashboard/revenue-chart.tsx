...
*/}
          {revenue.map((data) => (
            <div key={data.month} className="flex flex-col items-center gap-2">
              <span className='text-blue-600'>${data.revenue}</span>
              <div className="w-full rounded-md bg-blue-300" style={{ height: `${(chartHeight / topLabel) * data.revenue}px`, }}  ></div>
              <p className="-rotate-90 text-sm text-gray-400 sm:rotate-0">  {data.month}  </p>
            </div>
          ))}
        </div>
